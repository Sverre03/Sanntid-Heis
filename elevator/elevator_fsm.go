...
func FsmOnRequestButtonPress(elev Elevator, btnFloor int, btnType ButtonType, inTimer *timer.Timer) {
	...
	case EB_DoorOpen:
		...
		if RequestsShouldClearImmediately(elev, btnFloor, btnType) {
			timer.TimerStart(inTimer, config.DOOR_OPEN_DURATION)
		} else {
...
func FsmOnFloorArrival(elev Elevator, newFloor int, inTimer *timer.Timer) {
...
func FsmOnDoorTimeout(elev Elevator, inTimer *timer.Timer) {
	fmt.Printf("\n\n%s()\n", "fsmOnDoorTimeout")
	PrintElevator(elev)
